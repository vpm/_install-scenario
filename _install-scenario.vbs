...
If (((CurrentVersion = "6.3") Or (CurrentVersion = "6.4")) And (PROCESSOR_ARCHITECTURE = "AMD64")) Then
	...
	objShell.Run "powershell New-ItemProperty -Force -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\NcdAutoSetup\Private -Name AutoSetup -Value 0", 2, true

	' Show desktop icons
	objShell.Run "powershell New-ItemProperty -Force -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel -Name '{20D04FE0-3AEA-1069-A2D8-08002B30309D}' -Value 0", 2, true
	objShell.Run "powershell New-ItemProperty -Force -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel -Name '{5399E694-6CE5-4D6C-8FCE-1D8870FDCBA0}' -Value 0", 2, true
	objShell.Run "powershell New-ItemProperty -Force -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel -Name '{59031a47-3f72-44a7-89c5-5595fe6b30ee}' -Value 0", 2, true
	objShell.Run "powershell New-ItemProperty -Force -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel -Name '{645FF040-5081-101B-9F08-00AA002F954E}' -Value 0", 2, true
	objShell.Run "powershell New-ItemProperty -Force -Path HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel -Name '{F02C1A0D-BE21-4350-88B0-7367FC96EF3C}' -Value 0", 2, true
	...
	objShell.Run "powershell Set-TimeZone -Id 'FLE Standard Time' -PassThru", 2, true
	...
	If Not (InstallationType = "Server") Then
		...
		' Uninstall OneDrive
		objShell.Run "winget uninstall --name ""Microsoft OneDrive"" --accept-source-agreements --force", 2, true

		' Uninstall Microsoft Teams
		objShell.Run "winget uninstall --name ""Microsoft Teams"" --accept-source-agreements --force", 2, true

		' Uninstall Outlook for Windows
		objShell.Run "winget remove --name ""Outlook for Windows"" --accept-source-agreements --silent", 2, true

		' Uninstall Xbox
		objShell.Run "winget remove --name ""Xbox"" --accept-source-agreements --silent", 2, true

		' Uninstall Power Automate
		objShell.Run "winget remove --name ""Power Automate"" --accept-source-agreements --silent", 2, true

		' Uninstall Microsoft 365 Copilot
		objShell.Run "winget remove --name ""Microsoft 365 Copilot"" --accept-source-agreements --silent", 2, true
		...
		filePath = strProgFiles & "\OfficeDeploymentTool\configuration-OfficeStandard-x64.xml"
		Set file = fso.CreateTextFile(filePath, True)

		file.WriteLine "<Configuration>"
		file.WriteLine " <Add OfficeClientEdition=""64"" Channel=""PerpetualVL2024"">"
		file.WriteLine "  <Product ID=""Standard2024Volume"">"
		...
		' Install Office Standard
		objShell.Run "powershell Start-Process -WindowStyle Minimized -Wait -WorkingDirectory ${env:ProgramFiles}\OfficeDeploymentTool -FilePath setup.exe -ArgumentList '/configure configuration-OfficeStandard-x64.xml'", 2, true
	...
	' Install Microsoft.VCRedist.2015+.x86
	objShell.Run "winget install Microsoft.VCRedist.2015+.x86", 2, true

	' Install Microsoft.VCRedist.2015+.x64
	objShell.Run "winget install Microsoft.VCRedist.2015+.x64", 2, true
	...
	' reboot
	objShell.Run "shutdown /r /t 16 /f", 2, true
